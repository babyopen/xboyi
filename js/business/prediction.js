...
export const prediction = {
  ...
  initMLServiceChecker: () => {
    try {
      // 延迟5秒后开始检查ML服务状态，避免页面加载时的错误
      setTimeout(() => {
        prediction.checkMLServiceStatus();
      }, 5000);
      ...
      console.debug('ML服务检查初始化失败:', e.message);
    }
  },

  checkMLServiceStatus: async () => {
    try {
      // 使用Promise.race实现超时，兼容性更好
      const timeoutPromise = new Promise((_, reject) => {
        setTimeout(() => reject(new Error('Timeout')), 2000);
      });
      
      const fetchPromise = fetch('http://localhost:8000/ml-api/api/health', {
        method: 'GET'
      });
      ...
      if (response.ok) {
        prediction.updateMLServiceUI(true);
      } else {
  ...
  copyMLStartCommand: () => {
    const command = 'python3 python/ml_api_server.py';
  ...
  runMLPrediction: async () => {
    try {
      const response = await fetch('http://localhost:8000/ml-api/api/predict', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json'
        },
        body: JSON.stringify({
          history: StateManager._state.analysis.historyData.slice(0, 30)
        })
      });
      
      if (!response.ok) {
        throw new Error('ML预测请求失败');
      }
      
      const result = await response.json();
      prediction.displayMLPredictionResult(result);
      prediction.saveMLPredictionHistory(result);
    } catch(e) {
      console.error('ML预测失败:', e);
      Toast.show('ML预测服务不可用');
    }
  },

  displayMLPredictionResult: (result) => {
    const resultEl = document.getElementById('mlPredictionResult');
    const contentEl = document.getElementById('mlPredictionContent');
    const timeEl = document.getElementById('mlPredictionTime');
    
    if (resultEl && contentEl && result) {
      resultEl.style.display = 'block';
      timeEl.textContent = new Date().toLocaleTimeString();
      
      // 显示预测结果
      if (result.predictions && Array.isArray(result.predictions)) {
        const html = result.predictions.map((pred, index) => {
          return `<div style="margin:8px 0;padding:8px;background:rgba(255,255,255,0.2);border-radius:6px;">
            <span style="font-weight:bold;">${index + 1}. ${pred.zodiac}</span>
            <span style="float:right;opacity:0.9;">置信度: ${(pred.confidence * 100).toFixed(1)}%</span>
          </div>`;
        }).join('');
  ...
  saveMLPredictionHistory: (result) => {
    try {
      ...
      Storage.set('mlPredictionHistory', history);
    } catch(e) {
